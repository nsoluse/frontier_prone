...
Citizen.CreateThread(function()
	while true do 
		...
		if (DoesEntityExist(ped) and not IsPedDeadOrDying(ped)) then 
			...
			DisableControlAction(0, proneKey, true) 
			if (not IsPauseMenuActive()) then 
				if (IsDisabledControlJustPressed(0, proneKey) and not IsPedInAnyVehicle(ped, true) and not IsPedFalling(ped) and not IsPedDiving(ped) and not IsPedInCover(ped, false) and not IsEntityInWater(ped) and not Citizen.InvokeNative(0xA911EE21EDF69DAF, ped)) then
                    if proned then
						TaskPlayAnimAdvanced(PlayerPedId(), "mech_crawl@base", "stealth2idle", GetEntityCoords(PlayerPedId()), 0.0, 0.0, GetEntityHeading(PlayerPedId()), 1.0, -1.0, 1500, 0, 0, 0, 0)
						proned = false
					elseif not proned then
