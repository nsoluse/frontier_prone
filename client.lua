	...
	while true do 
		...
		if (DoesEntityExist(ped) and not IsPedDeadOrDying(ped)) then 
			...
			if (not IsPauseMenuActive()) then 
				if (IsDisabledControlJustPressed(0, proneKey) and not IsPedInAnyVehicle(ped, true) and not IsPedFalling(ped) and not IsPedDiving(ped) and not IsPedInCover(ped, false) and not IsEntityInWater(ped)) then -- Various kinds of checks
                    if proned then
